...
const MODE_ORDER = ['plan', 'qq', 'wechat'];
...
function renderMarkdown(src) {
  ...
  const out = [];
  let inTable = false, listType = null, listOpen = false;
  const closeList = () => { if (listOpen) { out.push('</' + listType + '>'); listOpen = false; listType = null; } };
  for (let line of lines) {
    const t = line.trim();
    if (!t) { closeList(); inTable = false; out.push(''); continue; }
    ...
    if (/^(-{3,}|\*{3,}|_{3,})$/.test(t)) { closeList(); inTable = false; out.push('<hr/>'); continue; }
    if (t.startsWith('>')) {
      closeList(); inTable = false;
    ...
    if (ul || ol) {
      inTable = false;
      const type = ul ? 'ul' : 'ol';
      if (!listOpen || listType !== type) { closeList(); out.push('<' + type + '>'); listOpen = true; listType = type; }
    ...
    if (t.startsWith('|')) {
      ...
      if (!isSep) out.push('<tr>' + cells.map((c) => '<td>' + renderInline(c) + '</td>').join('') + '</tr>');
      continue;
...
function buildModeTabs() {
  ...
  for (const m of MODE_ORDER) {
    ...
    b.innerHTML = `<span class="mi">${d.icon}</span><span class="ml">${d.label}</span><span class="md">${d.modeName}</span>`;
...
function buildUserMessage(obj, note) {
  ...
  if (state.mode === 'plan') {
    ...
    L.push('\n要求：严格套用固定模板结构（封面→背景→目的→时间→地点→对象→内容→流程→应急→预算→落款→附件），使用学院特有公文话术与政治表述，并在结尾附"待确认清单"。');
...
async function applySelectionRevise(ctx) {
  ...
  await streamChat(state.mode, [{
    ...
    refreshPreview();
    editor.focus();
    const np = before.length + clean.length;
    try { editor.setSelectionRange(before.length, np); } catch (e) {}
    updateContextButtons();
  });
  hideRevise();
  toast('选中片段已修改', 'ok');
...
async function runPic(ctx) {
  ...
  await streamChat('pic', [{
    role: 'user',
    content: '以下是一篇已完成的新媒体推送文案，请结合该推送与活动内容，为它生成配图图片提示词：\n目标平台/部门参考：' + platform + '\n\n【已完成推送文案】\n' + pushText + '\n\n请按技能输出 1 张承载活动主题的固定主图 + 若干辅助配图的分组提示词，注明图序、画面描述与比例；参数未定处用默认并在末尾标注。'
  }], (full) => { editor.value = prefix + full; refreshPreview(); });
  toast('配图提示词已生成', 'ok');
}
...
function extractChecklist(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (/待确认清单|待确认|待核对|请核对/.test(lines[i])) { start = i; break; }
  }
  if (start < 0) return [];
  const items = [];
  for (let i = start + 1; i < lines.length; i++) {
    ...
    if (!line) continue;
    if (/^#{1,4}\s/.test(line) || /^-{3,}$/.test(line) || /^\*{3,}$/.test(line)) break;
    // 去掉列表标记与编号
    ...
    if (line.startsWith('|')) {
      ...
      if (!cells.length || cells.every((c) => /^:?-{1,}:?$/.test(c))) continue;
      line = cells.join('｜');
    }
    // 过滤表头/表头行、注释说明
    if (/^(序号|项目|事项|名称|金额|待确认|项)/.test(line) && line.length < 8) continue;
    if (/至少包括|需人工|待核对|以下为|清单如下|：$/.test(line)) { items.push(line); continue; }
    items.push(line);
  }
...
function saveCurrentFromModal() {
  ...
  if (state.editingLibId) {
    ...
    if (it) { it.title = title; it.memo = memo; it.text = editor.value; it.mode = state.mode; it.updated = Date.now(); }
  } else {
    arr.unshift({
      id: libId(), title, memo, mode: state.mode,
      text: editor.value,
...
function renderLib() {
  ...
  arr.forEach((it) => {
    ...
    btns.appendChild(mkBtn('载入', () => { editor.value = it.text; refreshPreview(); switchMode(it.mode); toast('已载入：' + it.title, 'ok'); }, 'primary'));
...
$('btnPic').onclick = guardRun(runPic);
...
// 弹窗按 Esc 关闭 & 点背景关闭
[['saveModal'], ['checkModal'], ['settingsModal']].forEach(([id]) => {
  const m = $(id);
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.key === 'Escape') {
    hideRevise();
    ['saveModal', 'checkModal', 'settingsModal'].forEach((id) => $(id).classList.add('hidden'));
  }
});
